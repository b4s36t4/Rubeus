...
import { getFromKCache, storeInKCache } from '../kreacher';
import { getHeaderObj } from '../kreacher/helpers';
...
export const getFromCache = async (
  ...
  try {
    ...
    const response = await getFromKCache(env, c, incomingRequestBody);
    if (response) {
      if (
    ...
    logger.error({
      message: `getFromCache error: ${error.message}`,
    });
    return [null, null, null];
  }
};
...
export const putInCache = async (
  ...
  try {
    ...
    await storeInKCache(env, c, incomingRequestBody);
  } catch (error: any) {
    logger.error({

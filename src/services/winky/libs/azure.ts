...
export async function uploadToAzureStorage(
  ...
  const url = `https://${accountName}.blob.core.windows.net/${containerName}/${filePath}`;
  ...
  if (!accessToken) {
    ...
    const canonicalizedResource = `/${accountName}/${containerName}/${filePath}`;
...
export async function getAzureAccessToken({
  ...
  if (azureAuthMode === 'entra') {
    ...
    //TODO: cache it
    return getAccessTokenFromEntraId(tenantId, clientId, clientSecret, scope);
  } else if (azureAuthMode === 'managed') {
    //TODO: cache it
    return getAzureManagedIdentityToken(resource);
  }
}
